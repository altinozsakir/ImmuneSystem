using UnityEngine; using UnityEngine.UI; using TMPro; using Core.Perks;
...
void Awake(){ Instance = this; if (!manager) manager = FindAnyObjectByType<PerkManager>(); gameObject.SetActive(false); }


public static void TryOpen(PerkManager m)
{
if (!Instance) return; Instance.manager = m; Instance.Open();
}
